...
class EccentricityUncertainty(TruthMethod):
    ...
    def __str__(self):
        return f'Eccentricity Uncertainty with {self.method_for_similarity} similarity method. Model for entailment: {self.model_for_entailment.config._name_or_path}, Tokenizer for entailment: {self.tokenizer_for_entailment}, Number of generations: {self.number_of_generations}, Threshold: {self.threshold}, Standard Deviation: {self.std}, temperature: {self.temperature}, eigenvalue threshold: {self.eigen_threshold}.'
